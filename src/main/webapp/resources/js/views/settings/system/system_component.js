 ...
 * 
 * Application Name : 컴포넌트 관리[SystemComponentApp]
 * Program Code     : PC0029
 * Description      :
 * Revision History
 ...
 */
var SystemComponentApp = function () {
	"use strict";
	...
	// [El]프로그램 그리드
	var $componentGrid = $("#systemComponentGrid");
	
	// [Data]local 컴포넌트 콤보박스 데이터
    ...
    function fnComponentEvents(){
    	...
    	$("#langProgramCode, #langCompId, #langCompNm").enterEvent({
    		callBack:function(value){
    			...
    			$componentGrid.paragonGridSearch(data);
    		}
    	})
    	
    	//저장버튼
    	$("#componentSaveRowBtn").click(function(){
    		fnSaveComponentRows();
    	});
    	//행추가버튼
    	$("#componentAddRowBtn").click(function(){
    		$componentGrid.appendRow();
    	});
    	//검색버튼
    	$("#componentSearchBtn").click(function(){
    		fnSearchListProgram();
    	});
    	//행삭제버튼
    	$("#componentDelRowBtn").click(function(){
    		$componentGrid.rowDel();
    	});
    }
    ...
    function fnSearchListProgram(){
    	...
    	if(fnModCheck()){
	    	var data = {
					PRO_CD : $("#componentCode").val(),
					PRO_NM : $("#componentName").val()
			};
    		$componentGrid.paragonGridSearch(data);
    	}
    }
    ...
    function fnGetProgramNms(data){
    	$.ajax({
    		...
    		success : function(result) {
    			...
    			//드릴다운 부분을 닫기 위함
    			$("#componentName").autocomplete("search", "");
    			$("#componentName").val("").focus();
    		}
    	});
    }
    /********************************************************************
     ...
     ********************************************************************/
    //[Fn] grid 컴포넌트관리 목록 
    function fnListComponent(){
		$componentGrid.paragonGrid({
        	url: '/ctrl/settings/system/component/listComponent',
			sortable: true,
			componentId: "CP0011",
			rowEditable : true,
            colModel:[
                {name:'COMP_SEQ', align:"center",hidden:true},
                {editable: true,name:'PRO_CD', align:"center",width:80, rowspan:true},
                {name:'PRO_NM', align:"center",width:80, rowspan:true},
                {editable: true,name:'COMP_ID', align:"center",width:80},
                {editable: true,name:'COMP_NM', align:"center"},
                {
                	editable: true, 
		        	align:"center",
		        	name:'COMP_GBN',
                ...
                {editable: true,name:'COMP_DESC',width:700},
                {
			    	editable: true, 
			    	name:'USE_YN', 
			    	align:"center",
    ...
    //그리드 수정 여부 체크
    function fnModCheck(){
    	return $componentGrid.paragonGridModConfirm("수정된 내용이 있습니다. 계속 진행하시겠습니까?");
	}
    //[Fn] 수정된 내용저장
    function fnSaveComponentRows() {
		...
		// grid에서 formatData에 정의된 데이터를 'dt_component' 키값으로 parsing
    	var jsonData = $componentGrid.getJsonData("dt_component",formatData);
		...
		$.ajax({
    		url : "/ctrl/settings/system/component/saveComponent",
    		data :jsonData,
    		contentType: 'application/json; charset=utf-8',
    		cache: false,
    		success : function(result) {
    			alert(result.msgTxt);
    			$componentGrid.trigger("reloadGrid");
    		}
    	});
