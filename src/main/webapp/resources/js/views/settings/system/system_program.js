 ...
 * 
 * Application Name : 언어 관리[SystemProgramApp]
 * Program Code     : PC0001
 * Description      :
 * Revision History
...
var SystemProgramApp = function () {
	...
	// [El]프로그램 그리드
	var $programGrid = $("#systemProgramGrid");
    ...
    function fnProgramEvents(){
    	...
    	$("#programName").enterEvent({
    		callBack:function(value){
    			...
    			if(fnModCheck()){
    				var data = {
    						proCd : $("#programCode").val(),
    						proNm : $("#programName").val()
    				};
    				$programGrid.search(data);
    ...
    function fnListProgram(){
		$programGrid.paragonGrid({
            ...
            colModel:[
                ...
                {editable: true, name:'PRO_DESC',maxLength:10},
    ...
    //그리드 수정 여부 체크
    function fnModCheck(){
    	return $programGrid.paragonGridModConfirm("수정된 내용이 있습니다. 계속 진행하시겠습니까?");
	}
    ...
    function saveRows() {
    	...
    	var jsonData = $programGrid.getJsonData("dt_program",rowData);
