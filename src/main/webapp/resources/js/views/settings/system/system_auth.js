...
var SystemAuthApp = function () {
    ...
    return {
        init: function () {
        	...
        	$("#systemAuthGroupModBtn").remove();
        	$("#systemAuthGroupAddBtn").remove();
	    },
	    initPopup: function () {
    	...
    	$.ajax({
    		...
    		success : function(result) {
    			for (var i = 0; i < result.length; i++) {
    				...
    				var userType 	= result[i].USER_TYPE;
    				
    				var option = $("<option>", {value: userId});
    				var thisName = userId+' - '+userNm+' ['+userType+']';
    ...
    function fnListAuthSearchUser(searchWord){
    	...
    	$.ajax({
    		...
    		data :JSON.stringify({searchWordArr : searchWordArr, authGroupSeq: authGroupSeq}),
    		type : "POST",
    		dataType : "json",
    		contentType: 'application/json; charset=utf-8',
    		success : function(result) {
    ...
    function fnListAuthGroupSelectBox(authGroupSeq){
    	$.ajax({
    		...
    		success : function(result) {
    	    	...
    	    	select.html('');
    	    	for (var i = 0; i < result.length; i++) {
    	    		var thisValue = result[i].value                                    
    	    		var thisName = result[i].name;
    ...
    function fnListAuth(authGroupSeq){
		$systemAuthGrid.paragonGrid({
			...
			customModel:[
			    ...
			    {name:"AUTH_CD",hidden:true},
			    {name:"AUTH_GROUP_SEQ",hidden:true},
			    {align:"left",name:'MENU_NM',sortable:false},
			    {align:"center",name:'PRO_CD',sortable:false}
            ],
            customMode :"checkbox",  
//            gridResize :null,		 
//            height:"auto",			
    ...
    function fnSaveAuthGroup() {
		...
		$.ajax({
    		...
    		success : function(result) {
    			...
    			$("#authGroupSave").paragonClosePopup();
