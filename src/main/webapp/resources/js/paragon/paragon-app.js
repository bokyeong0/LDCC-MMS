		...
		callBackCasting: function (data) {
			if(callBackEls){
				$.each( callBackEls, function( key, targetKey ) {
					if($("#"+targetKey).is("input")){
						$("#"+targetKey).val(data[key]);
...
//		},
		...
		prcsStart: function(msg, time) {
			...
			setTimeout(function() {
				if(!$('#ajax-loader').hasClass("hide")){
					$('#ajax-loader').addClass('hide');
					$('#ajax-text').addClass('hide');
					$('#ajax-text').html("");
				}
            }, settime);
		},
		prcsFnStart: function(calBack,msg,time) {
			var settime = 2000;
			if(time){
				settime = time;
			}
